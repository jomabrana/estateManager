...
// ── HOUSE/UNIT NUMBER SEARCH ───────────────────────────────────

async function searchByHouseNumber() {
  const houseInput = document.getElementById("house-search");
  ...
  const tbody = document.getElementById("invoices-tbody");
  if (tbody) {
    tbody.innerHTML = `<tr><td colspan="10" class="table-loading">Searching for house ${unitNumber}...</td></tr>`;
  }

  try {
    const token = localStorage.getItem("token");
    const res = await fetch(`/api/invoices/search/by-unit?unitNumber=${encodeURIComponent(unitNumber)}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (!data.invoices || data.invoices.length === 0) {
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="10" class="table-loading" style="color:var(--danger);">No invoices found for house ${unitNumber}</td></tr>`;
      }
      showNotification(`No invoices found for house ${unitNumber}`, "info");
      return;
    }
    ...
    showNotification(`Found ${data.invoices.length} invoice(s) for house ${data.unitNumber}`, "success");
  } catch (err) {
    if (tbody) {
      tbody.innerHTML = `<tr><td colspan="10" class="table-loading" style="color:var(--danger)">${err.message}</td></tr>`;
    }
...
function handleHouseSearchKeypress(event) {
  if (event.key === "Enter") {
    searchByHouseNumber();
  }
}
