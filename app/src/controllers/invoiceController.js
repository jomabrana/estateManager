...
// ═══════════════════════════════════════════════════════════════════════════════
// GET /api/invoices/search/by-unit?unitNumber=XXX
// Search invoices by house/unit number — returns all payment invoices for a unit
// ═══════════════════════════════════════════════════════════════════════════════
const getInvoicesByUnitNumber = async (req, res) => {
  try {
    const estateId = await requireEstate(req.user.userId);
    const { unitNumber } = req.query;

    if (!unitNumber) {
      return res.status(400).json({ error: "unitNumber query parameter is required" });
    }
    ...
    const unit = await prisma.unit.findFirst({
      where: {
        estateId,
        unitNumber: { mode: "insensitive", equals: unitNumber }
      }
    });

    if (!unit) {
      return res.status(404).json({ invoices: [], message: "House/Unit not found" });
    }
    ...
    return res.json({ invoices, unitNumber: unit.unitNumber, totalCount: invoices.length });
  } catch (err) {
    if (err.message === "NO_ESTATE")
      return res.status(400).json({ error: "Your account is not linked to an estate" });
    console.error("Search invoices by unit error:", err);
    return res.status(500).json({ error: "Server error" });
  }
};
